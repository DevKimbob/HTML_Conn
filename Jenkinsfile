	...
	stage('Check for Version Tag') {
	  steps {
		script {
		  ...
		  if ("$OVERRIDE" == "0" && versionTag.contains("$VERSION")) {
			echo "$VERSION tag found. Exiting pipeline."
			return
		  }
		}
	  }
    ...
    stage('Kubernetes') {
      steps {
        script {
		  ...
		  sh "kubectl --kubeconfig=/root/admin.yaml get deployments.apps -n dev -o name | xargs -I{} kubectl --kubeconfig=/root/admin.yaml rollout restart -n dev {}"
        }
      }
    }
  }
